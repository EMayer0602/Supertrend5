...
import system  # Assuming this is a custom module you've imported elsewhere
...
class TradingSystem:
	...
	def calculate_trade_statistics(self, trades, equity_curve):
	   ...
	   # Ensure PnL is accessed as a scalar
		winning_trades = len([t for t in trades if float(t['PnL'].iloc[0]) > 0])
		losing_trades = len([t for t in trades if float(t['PnL'].iloc[0]) <= 0])
		
		profits = [float(t['PnL'].iloc[0]) for t in trades if float(t['PnL'].iloc[0]) > 0]
		losses = [float(t['PnL'].iloc[0]) for t in trades if float(t['PnL'].iloc[0]) <= 0]
...
'''		fig.add_trace(
		...
		'''
		end_date = datetime.now()
		start_date = end_date - timedelta(days=365)  # Approximately 120onths
...
# Add slider to equity chart
		...
		if long_trades:
			long_entries = [trade['Entry Date'] for trade in long_trades if trade['Entry Date'] >= plot_df.index[15]]
			long_entry_prices = [trade['Entry Price'] for trade in long_trades if trade['Entry Date'] >= plot_df.index[15]]
			long_exits = [trade['Exit Date'] for trade in long_trades if trade['Exit Date'] >= plot_df.index[15]]
			long_exit_prices = [trade['Exit Price'] for trade in long_trades if trade['Exit Date'] >= plot_df.index[15]]
		...
		if short_trades:
			short_entries = [trade['Entry Date'] for trade in short_trades if trade['Entry Date'] >= plot_df.index[15]]
			short_entry_prices = [trade['Entry Price'] for trade in short_trades if trade['Entry Date'] >= plot_df.index[15]]
			short_exits = [trade['Exit Date'] for trade in short_trades if trade['Exit Date'] >= plot_df.index[15]]
			short_exit_prices = [trade['Exit Price'] for trade in short_trades if trade['Exit Date'] >= plot_df.index[15]]
...
'''		# Add short trade entry and exit points
		if short_trades:
			...
			short_entry_prices = [trade['Entry Price'] for trade in short_trades]
		...
		if len(long_equity) > 0:
			self._add_equity_curve(fig, long_equity, 'Long Equity', 'green', 2, 1)
		if len(short_equity) > 0:
			self._add_equity_curve(fig, short_equity, 'Short Equity', 'red', 2, 1)
